...
function Ilustracion({ tipo }) {
  if (tipo === 'form') return <div className="guide-mock"><div className="guide-mock__bar"><b>Congregación</b><i className="fas fa-edit" /></div><label>Nombre</label><div className="guide-input">Congregación Central</div><div className="guide-mock__row"><div><label>Día de reunión</label><div className="guide-input">Miércoles</div></div><div><label>Salas</label><div className="guide-input">A y B</div></div></div><span className="guide-button">Guardar</span></div>
  ...
  return <div className="guide-mock"><div className="guide-mock__bar"><b>Editar asignaciones</b><span className="guide-round solid"><i className="fas fa-save" /></span></div>{[['Presidente', 'Daniel López'], ['Lectura de la Biblia', 'Sofía García'], ['Primera conversación', 'Ana Martínez']].map(([a, b], i) => <div className="guide-assignment" key={a}><span className="guide-assignment__number">{i + 1}</span><div><small>{a}</small><b>{b}</b></div><span className="guide-room">Sala A</span></div>)}</div>
}
