...
const bytes = await fs.readFile('C:/Users/carlo/Downloads/mwb_S_202607.pdf'); const file={arrayBuffer:async()=>bytes.buffer.slice(bytes.byteOffset, bytes.byteOffset+bytes.byteLength)}; const lines=await extractPdfLines(file); for(const p of [10,11,12,13]){ console.log('---PAGE',p,'---'); for(const l of lines.filter(l=>l.page===p && (/TESOROS|SEAMOS|NUESTRA|MEJORES|^\d+\./.test(l.text)||/JER/.test(normalizeHeader(l.rawText))))) console.log(l.column,Math.round(l.y),JSON.stringify(l.text),'raw',JSON.stringify(l.rawText)); }