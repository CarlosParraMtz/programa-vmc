...
function groupItemsIntoLines(items, pageNumber) {
  ...
  const columns = [
    bodyItems.filter((item) => item.x < 365),
    bodyItems.filter((item) => item.x >= 365),
  ];
...
const bytes = await fs.readFile('C:/Users/carlo/Downloads/mwb_S_202607.pdf'); const file={arrayBuffer:async()=>bytes.buffer.slice(bytes.byteOffset, bytes.byteOffset+bytes.byteLength)}; const lines=await extractPdfLines(file); for(const l of lines.filter(l=>l.page===3 && (l.text.includes('NUESTRA')||l.text.includes('VIDA')||/^7\./.test(l.text)||/^8\./.test(l.text)))) console.log(l.page,l.column,Math.round(l.y),JSON.stringify(l.text), parseAssignmentStart(l.text));