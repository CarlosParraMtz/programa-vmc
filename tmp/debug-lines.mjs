...
function parseMeetingHeader(text, year) {
  ...
  const studyMatch = header.match(/(JEREMIAS\s+.+?)(?:\s+\d+)?$/);
...
function groupItemsIntoLines(items, pageNumber) {
  ...
  const columns = [
    printableItems.filter((item) => item.x < 390),
    printableItems.filter((item) => item.x >= 390),
  ];
...
const lines = await extractPdfLines(file);
for (const l of lines.filter(l => l.page <= 3)) { if (/Canci|Canci/.test(l.rawText) || /Canci|Canci?n/.test(l.text) || /^d+./.test(l.text) || /TESOROS|SEAMOS|NUESTRA|VIDA/.test(l.text)) console.log(l.page, l.column, Math.round(l.y), JSON.stringify(l.rawText), '=>', JSON.stringify(l.text)); }